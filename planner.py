...
class WeeklyPlanner(tk.Tk):
    ...
    def create_task(self, task_name, grid_position):
        # Delete the menu widget that is created from right clicking.
        for widget in self.winfo_children():
        ...
        counter_all = 0
        label_locations = []
        # Count all widgets in the window.
        for widget in self.winfo_children():
            counter_all += 1
        difference = counter_all - 142
        # If there is no tasks in the current window first task is added to the first row of last column.
        if difference == 0:
            task.place(in_=self.grid_widgets[grid_position], relheight=0.99, relwidth=0.97)
        # New task added to the available spot in the "tasks" column.
        else:
